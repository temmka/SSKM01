 ...
 */
void modbusSlave::build_read_packet(uint8_t function, uint8_t count,
		uint8_t *packet) {
 ...
 */
void modbusSlave::build_write_packet(uint8_t function, uint16_t start_addr,
 ...
 */
void modbusSlave::build_write_single_packet(uint8_t function,
		uint16_t write_addr, uint16_t reg_val, uint8_t* packet) {
	packet[SLAVE] = slaveAddress;
	packet[FUNC] = function;
 ...
 */
void modbusSlave::build_error_packet(uint8_t function, uint8_t exception,
		uint8_t *packet) {
	packet[SLAVE] = slaveAddress;
...
int16_t modbusSlave::modbus_request(uint8_t *data) {
	...
	response_length = rcvByteCounter;
...
int16_t modbusSlave::validate_request(uint8_t *data, uint8_t length,
		uint16_t regs_size) {
	uint16_t i, fcnt = 0;
	uint16_t regs_num = 0;
...
int16_t modbusSlave::write_regs(uint16_t start_addr, uint8_t *query,
		u16 *regs) {
	int16_t temp;
	uint16_t i;
...
int16_t modbusSlave::preset_multiple_registers(uint16_t start_addr,
		uint8_t count, uint8_t *query, u16 *regs) {
...
int16_t modbusSlave::write_single_register(uint16_t write_addr, uint8_t *query,
		u16 *regs) {
...
int16_t modbusSlave::read_holding_registers(uint16_t start_addr,

uint8_t reg_count, u16 *regs) {
...
int16_t modbusSlave::update(u16 *regs, u16 regs_size) {
	...
	length = modbus_request(tBuffer);
	if (length < 1) {
		return length;
	}

	exception = validate_request(tBuffer, length, regs_size);

	if (exception) {
		build_error_packet(tBuffer[FUNC], exception, errpacket);
		send_reply(errpacket, EXCEPTION_SIZE);
		return (exception);
	}

#ifdef USE_COMUNICATION_LED
	MODBUS_LED_PORT->BSRRL = MODBUS_LED_PIN;
#endif
	start_addr = ((int) tBuffer[START_H] << 8) + (int) tBuffer[START_L];

	switch (tBuffer[FUNC]) {
	case FC_READ_REGS:

		return read_holding_registers(start_addr, tBuffer[REGS_L], regs);
		break;
	case FC_WRITE_REGS:

		return preset_multiple_registers(start_addr, tBuffer[REGS_L], tBuffer,
				regs);
		break;
	case FC_WRITE_REG:

		write_single_register(start_addr, tBuffer, regs);
		break;
	}
